    ...
    >
      <div className="d-flex align-items-center ">
        ...
        <div className="" style={{ width: '65%' }}>
          ...
          <h5 className="text-white text-center fw-bold mb-0">
            {' '}
            {tipo === 'porcentaje'
              ? '%'
...
const DashboardFinanciero = () => {
  ...
  if (cargando)
    return <div className="p-5 text-center">Cargando métricas...</div>;
  ...
  const volatilidad =
    utilidades.length > 0 ? ss.standardDeviation(utilidades) : 0;
  ...
  if (outliersReales.length > 0) {
    ...
    // Tu caso actual
    mensajeOutliers =
      'El flujo de ingresos actual es orgánico y sigue la tendencia del mercado.';
  }
  ...
  const sueldoSeguro =
    utilidadesFiltradas.length > 0 ? ss.quantile(utilidadesFiltradas, 0.2) : 0;
            ...
            {montado && stats.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value) => [
                      `${monedas.find((e) => e.value === moneda.campo)?.simbolo} ${Number(value).toLocaleString()}`,
                      '',
                    ]}
                    contentStyle={{
                      borderRadius: '10px',
          ...
          >
            {montado && datosFiltrados.length > 0 ? (
              <>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart width={600} height={300} data={datosFiltrados}>
                    ...
                    <XAxis
                      ...
                      axisLine={false}
                      tickLine={false}
                      tick={{ fill: '#ffffff', fontSize: 12 }} // Solo diseño
                    />
                    ...
                    <YAxis
                      ...
                      tickLine={false}
                      tick={{ fill: '#ffffff', fontSize: 12 }}
                      // AQUÍ es donde va la lógica de M y k
                      tickFormatter={(val) => {
                        if (val >= 1000000)
                    ...
                    <Tooltip
                      formatter={(value) =>
                        `${monedas.find((e) => e.value === moneda.campo)?.simbolo} ${value.toFixed(2)}`
                      }
                      contentStyle={{
                        backgroundColor: '#fff',
                        borderRadius: '10px',
                ...
                <div
                  className={`mt-3 p-2 rounded shadow-sm bg-light border-start border-4 ${dataConPrediccion.pendiente > 0 ? 'border-success' : 'border-danger'}`}
                >
                  <p
                    className={`mb-0  text-center bg-white ${dataConPrediccion.pendiente > 0 ? 'text-success' : 'text-danger'}`}
                    style={{ borderRadius: '5px', padding: '10px' }}
                  >
                    <FontAwesomeIcon icon={faRobot} className="me-2" />
                    Hola {localStorage.getItem('nombre')?.split(' ')[0]} tenemos
                    una{' '}
                    {dataConPrediccion.mensaje ||
                      'Situacion de datos insuficiente para mostrar un historico'}
                  </p>
                </div>
              </>
            ) : (
