...
export const useDashboard = () => {
  ...
  const predecirFuturoFinanciero = async (historico) => {
    ...
    const datosUtilidad = historico.map((h, index) => [
      index,
      h.total_ingresos - h.total_gastos,
    ]);
    ...
    let mensaje = 'Situacion de datos insuficiente para mostrar un historico';
    if (linea.m > 0) {
  ...
  const cargarDatos = useCallback(
    async (idMonedaManual) => {
      ...
      try {
        ...
        const data = await start(`${URL}comuun/stats-mensuales`, {
          moneda: idMoneda,
        });
